...
if __name__== "__main__":
    ...
    lines = sc.textFile("hdfs://master:54310/user/hduser/output.csv")
    ...
    def testForPass(value):
        if int(value)>0:
            return 1
        else:
            return 0
    points2 = data5.map(lambda x:(np.array([testForPass(x[1][0][D])] + x[1][0][:D],dtype=np.float32)))
    print(points2.collect())

    #DATA CLEANSING ABOVE^^ 
    ...
    #NEW WAY BELOW
    points = points2.map(lambda x: LabeledPoint(x[0], x[1:])).cache()
    model = LogisticRegressionWithLBFGS.train(points, iterations=100)
    ...
    print(str(model.predict(points.collect()[5].features))+str(points.collect()[5]))
    #model.save(sc, "target/tmp/pythonLogisticRegressionWithLBFGSModel")
    #sameModel = LogisticRegressionModel.load(sc, "target/tmp/pythonLogisticRegressionWithBFGSModel")
    ...
    '''def gradient(matrix, w):
            ...
            # For each point (x, y), compute gradient function, then sum these
            return ((1-(1.0 / (1.0 + np.exp(-Y * X.dot(w))))) * -Y * X.T).sum(1)
    ...
    for i in range(500):
        ...
        #print("POINTS: "+str(points.collect()))
        #w -= 0.01/200 * (np.sum(points.map(lambda m: gradient(m,w)).reduce(lambda x, y : np.concatenate((x,y), axis = 0)),axis=0))
        w -= 1000 * points.map(lambda m: gradient(m, w)).reduce(add)
